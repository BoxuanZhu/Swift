...
class ViewController: UIViewController {
    ...
    @IBAction func parkBtnPressed(_ sender: Any) {
        if mapView.annotations.count == 1{
            ...
            mapView.removeAnnotation(parkedCarAnnotation!)
            parkBtn.setImage(UIImage(named: "parkCar.png"), for: .normal)
        }
        centerMapLocation(location: LocationService.instance.locationManager.location!)
    }
...
extension ViewController: CLLocationManagerDelegate{
    ...
    func mapView(_ mapView: MKMapView, didUpdate userLocation: MKUserLocation) {
        centerMapLocation(location: CLLocation(latitude: userLocation.coordinate.latitude, longitude: userLocation.coordinate.longitude))
        let locationServiceCoordinate = LocationService.instance.locationManager.location!.coordinate
        parkedCarAnnotation = ParkingSpot(title: "My Parked Spot", locationName: "Tap the i for GPS", coordinate: CLLocationCoordinate2D(latitude: locationServiceCoordinate.latitude, longitude: locationServiceCoordinate.longitude))
    }
}
